    ...
    if request.method == 'POST':
        ...
        if password == password2:
            ...
            user = User.objects.create_user(username=username, password=password, email=email, organization=organization,
                                            country=country, city=city, department=department, address=address, phone=phone)
...
def mailGun(request):
    if request.method == 'POST':
        ...
        msg.send()
        messages.success(request, 'Dear ' + usernameTouch +
                         ', Great! We will be in contact soon.')
